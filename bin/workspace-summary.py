#!/usr/bin/env python3
"""
Check git status across multiple working directories with a common prefix.
Provides a summary of changes with special focus on requirements directory.
Creates symlinks for easy navigation.

Usage:
    workspace-summary.py myproject           # Show status of all myproject.* workspaces
    workspace-summary.py myproject --find-empty  # Find first empty workspace
    workspace-summary.py myproject --symlink-prefix mp  # Create ~/src/mp.<taskname> symlinks
"""
...
def create_symlink(src_dir, workspace_dir, task_name, symlink_prefix):
    """Create a symlink ~/src/<prefix>.<taskname> -> ~/src/<project>.X"""
    if not task_name:
        return False

    symlink_path = src_dir / f"{symlink_prefix}.{task_name}"

    try:
        # Clean up any old symlinks pointing to this workspace directory
        for potential_link in src_dir.glob(f"{symlink_prefix}.*"):
            if potential_link.is_symlink():
                try:
                    if potential_link.resolve() == workspace_dir.resolve():
                        # This symlink points to our workspace_dir, remove it if it's not the one we want
        ...
        if symlink_path.is_symlink() or symlink_path.exists():
            ...
            if symlink_path.is_symlink() and symlink_path.resolve() == workspace_dir.resolve():
                return False  # Already correct
            symlink_path.unlink()

        # Create new symlink
        symlink_path.symlink_to(workspace_dir)
        return True
...
def find_empty_workspace(workspace_dirs):
    """Find the first workspace with a clean working directory."""
    for workspace_dir in workspace_dirs:
        if not workspace_dir.is_dir():
            continue

        summary = get_status_summary(workspace_dir)
...
def main():
    parser = argparse.ArgumentParser(
        description="Check status of working directories with a common prefix"
    )
    parser.add_argument(
        "project_prefix",
        help="Project prefix to search for (e.g., 'myproject' will find myproject.*)"
    )
    parser.add_argument(
        "--find-empty",
        action="store_true",
    ...
    parser.add_argument(
        "--symlink-prefix",
        help="Prefix for symlinks (default: first 2 chars of project_prefix)"
    )
    args = parser.parse_args()

    src_dir = Path.home() / "src"

    # Determine symlink prefix
    if args.symlink_prefix:
        symlink_prefix = args.symlink_prefix
    else:
        # Default to first 2 characters
        symlink_prefix = args.project_prefix[:2]

    # Find all <project_prefix>.* directories
    workspace_dirs = sorted(src_dir.glob(f"{args.project_prefix}.*"))

    if not workspace_dirs:
        print(f"No {args.project_prefix}.* directories found in ~/src", file=sys.stderr)
        return 1

    # Handle --find-empty flag
    if args.find_empty:
        empty_workspace = find_empty_workspace(workspace_dirs)
        if empty_workspace:
    ...
    print("=" * 80)
    print(f"{args.project_prefix.upper()} - PARALLEL WORKING DIRECTORIES STATUS")
    print("=" * 80)
    print()
    ...
    for workspace_dir in workspace_dirs:
        ...
        summary = get_status_summary(workspace_dir)
        server_port = check_local_server(workspace_dir)

        # Create symlink for this directory
        task_name = extract_task_name(branch)
        if task_name:
            if create_symlink(src_dir, workspace_dir, task_name, symlink_prefix):
                symlinks_created.append(f"{symlink_prefix}.{task_name}")
        ...
        if task_name:
            header += f" → ~/src/{symlink_prefix}.{task_name}"
